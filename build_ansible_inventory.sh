...
GREP_COUNT=$(grep -E -c 'x\.x\.x\.x|z\.z\.z\.z' ../Ansible/inventory.yml)
if [ "$GREP_COUNT" -ne 3 ]; then
  echo "This script is expecting the default values of x.x.x.x, and z.z.z.z for the dc, and win10 hosts respectively in Ansible/inventory.yml."
...
sed -i.bak "s/x.x.x.x/$DC_IP/g; s/z.z.z.z/$WIN10_IP/g" ../Ansible/inventory.yml
